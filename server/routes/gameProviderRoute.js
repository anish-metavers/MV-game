...
router.get(
   '/get-games-providers',
   varifyJwtToken,
   gameProviderController.getAllGameProviders
);
router.get(
   '/get-single-game-provider',
   varifyJwtToken,
   gameProviderController.getSingleGameProvider
);
router.get(
   '/get-game-providers-games',
   varifyJwtToken,
   gameProviderController.getProvidersGames
);
...
router.post(
   ...
   upload.single('file'),
   gameProviderDocValidation,
   gameProviderController.createNewGameProvider
);
...
router.patch(
   ...
   upload.single('file'),
   gameProviderDocValidation,
   gameProviderController.updateGameProvider
);
router.patch(
   '/block-single-game-provider',
   varifyJwtToken,
   gameProviderController.blockSingleGameProvider
);
router.patch(
   '/unblock-single-game-provider',
   varifyJwtToken,
   gameProviderController.unblockSingleGameProvider
);
